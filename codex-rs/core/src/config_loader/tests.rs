...
#[tokio::test]
async fn profile_developer_instructions_file_sets_developer_instructions() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let codex_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&codex_home).await?;
    tokio::fs::write(
...
[profiles.work]
developer_instructions_file = "instr.md"
"#,
    )
    .await?;

    tokio::fs::write(
        codex_home.join("instr.md"),
    ...
    assert_eq!(
        config.developer_instructions.as_deref(),
        Some("profile developer instructions")
    );
